...
const { app } = require('electron') as {
  app: { whenReady(): Promise<void>; quit(): void };
};
...
type NapiModule = typeof import('../app/mailcore-rs/index');
...
app.whenReady().then(async () => {
  ...
  let addon: NapiModule;
  try {
    addon = require(path.join(__dirname, '..', 'app', 'mailcore-rs', 'loader.js')) as NapiModule;
    console.log('PASS  Wrapper module loaded');
  } catch (e) {
    console.error('FAIL  Wrapper module failed to load:', (e as Error).message);
    app.quit();
    process.exit(1);
  }
  ...
  try {
    ...
    results.push({ name, ok, error: error ?? undefined });
    if (ok) {
      console.log(`PASS  ${name}`);
    } else {
  ...
  check('username fallback: empty/null imap_username falls back to emailAddress', () => {
    ...
    // Cast through string|null to test runtime JS coercion (the || fallback pattern)
    const empty = '' as string | null;
    const nil = null as string | null;
    const custom = 'custom@example.com' as string | null;
    assert((empty || emailAddress) === emailAddress, 'empty string must fall back to emailAddress');
    assert((nil || emailAddress) === emailAddress, 'null must fall back to emailAddress');
    assert((custom || emailAddress) === 'custom@example.com', 'non-empty username must not be overridden');
  });

  // Summary
  const passedCount = results.filter(r => r.ok).length;
  const total = results.length;
  console.log(`\nResults: ${passedCount}/${total} passed`);
