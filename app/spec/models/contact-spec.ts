...
describe('Contact', function() {
  ...
  it('can be built via the constructor', function() {
    ...
    expect(c1.email).toBe('evan@UnifyMail.com');
  });
  ...
  it("falls back to the first component of the email if name isn't present", function() {
    const c1 = new Contact({ name: ' Evan Morikawa ', email: 'evan@UnifyMail.com' });
    expect(c1.displayName()).toBe('Evan Morikawa');
    expect(c1.displayName({ compact: true })).toBe('Evan');

    const c2 = new Contact({ name: '', email: 'evan@UnifyMail.com' });
  ...
  it('properly parses names with @', function() {
    ...
    const c6 = new Contact({ name: 'ev@UnifyMail.com', email: 'ev@UnifyMail.com' });
    expect(c6.firstName()).toBe('ev@UnifyMail.com');
    expect(c6.lastName()).toBe('');

    const c7 = new Contact({ name: 'evan@UnifyMail.com' });
    expect(c7.firstName()).toBe('evan@UnifyMail.com');
    expect(c7.lastName()).toBe('');
  ...
  it('should properly return `You` as the display name for the current user', function() {
    ...
    expect(c1.displayName()).toBe('You (tester@UnifyMail.com)');
  ...
  describe('isMe', function() {
    it('returns true if the contact name matches the account email address', function() {
      ...
      c1 = new Contact({ email: 'ben@UnifyMail.com' });
  ...
  describe('isValid', function() {
    it('should return true for a variety of valid contacts', function() {
      expect(new Contact({ name: 'Ben', email: 'ben@UnifyMail.com' }).isValid()).toBe(true);
      expect(new Contact({ email: 'ben@UnifyMail.com' }).isValid()).toBe(true);
      expect(new Contact({ email: 'ben+123@UnifyMail.com' }).isValid()).toBe(true);
    });
    ...
    it('should return false if the contact has an email that is not valid', function() {
      expect(new Contact({ name: 'Ben', email: 'Ben <ben@UnifyMail.com>' }).isValid()).toBe(false);
      expect(new Contact({ name: 'Ben', email: '<ben@UnifyMail.com>' }).isValid()).toBe(false);
      expect(new Contact({ name: 'Ben', email: '"ben@UnifyMail.com"' }).isValid()).toBe(false);
    });
