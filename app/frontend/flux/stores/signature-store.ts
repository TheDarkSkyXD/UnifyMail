import { localized, Actions, AccountStore } from 'unifymail-exports';
...
class _SignatureStore extends UnifyMailStore {
  ...
  activate() {
    ...
    if (!this.signatures) {
      const sentFrom = localized('Sent from UnifyMail, the best free email app for work');
      this.signatures = {
        initial: {
          id: 'initial',
          ...
          body: `<div><div>${sentFrom.replace(
            'UnifyMail',
            '<a href="https://github.com/TheDarkSkyXD/UnifyMail">UnifyMail</a>'
          )}</div></div>`,
          data: {
            title: sentFrom,
