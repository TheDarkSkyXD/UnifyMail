...
export class MailsyncProcess extends EventEmitter {
  ...
  constructor({ configDirPath, resourcePath, verbose }) {
    ...
    // Check the native build output directory as a fallback.
  ...
  _spawnProcess(mode) {
    ...
    if (fs.existsSync(this.binaryPath)) {
      ...
      console.warn(`[Dev] mailsync binary not found at ${this.binaryPath}, falling back to mock`);
      // Migrate/reset modes need Electron's native module loader (e.g. better-sqlite3).
      // Sync and test modes use system node: sync because Electron's main process exits
      // prematurely for persistent child scripts, and test because spawning a second
      // Electron instance on Windows causes the parent process to exit unexpectedly.
      const needsElectron = mode === 'migrate' || mode === 'reset';
      const executable = needsElectron ? process.execPath : 'node';
      this._proc = spawn(executable, [mockPath, ...args], { env });
    } else {
      throw new Error(`mailsync binary not found at ${this.binaryPath} and no mock available`);
    }
    ...
    if (this.account && this._proc.stdout) {
      this._proc.stdout.once('data', () => {
        // Guard against EPIPE: short-lived modes (e.g. test) may exit before we
        // finish piping, so silently ignore write errors to a dead child's stdin.
        if (!this._proc || !this._proc.stdin || this._proc.killed) return;
  ...
  async test() {
    // Try N-API fast path for account validation (in-process, no child spawn)
    try {
      ...
      // N-API addon not available or failed — fall back to spawning mailsync process
