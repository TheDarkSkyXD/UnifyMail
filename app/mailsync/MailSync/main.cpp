...
void runListenOnMainThread(shared_ptr<Account> account) {
    ...
    while(true) {
        ...
        try {
            ...
            if (type == "detect-provider") {
                AutoreleasePool pool;
                string email = packet["email"].get<string>();
                string requestId = packet.count("requestId") ? packet["requestId"].get<string>() : "";
                MailProvider * provider = MailProvidersManager::sharedManager()->providerForEmail(AS_MCSTR(email));
                json resp = {{"type", "provider-result"}, {"requestId", requestId}};
                if (provider) {
                    json providerJson = json::object();
                    if (provider->identifier()) {
                        providerJson["identifier"] = provider->identifier()->UTF8Characters();
                    }
                    json servers = json::object();
                    // Serialize IMAP services
                    json imapArr = json::array();
                    if (provider->imapServices()) {
                        for (unsigned int i = 0; i < provider->imapServices()->count(); i++) {
                            NetService * svc = (NetService *)provider->imapServices()->objectAtIndex(i);
                            json s = json::object();
                            if (svc->hostname()) s["hostname"] = svc->hostname()->UTF8Characters();
                            s["port"] = svc->port();
                            s["connectionType"] = svc->connectionType();
                            imapArr.push_back(s);
                        }
                    }
                    servers["imap"] = imapArr;
                    // Serialize SMTP services
                    json smtpArr = json::array();
                    if (provider->smtpServices()) {
                        for (unsigned int i = 0; i < provider->smtpServices()->count(); i++) {
                            NetService * svc = (NetService *)provider->smtpServices()->objectAtIndex(i);
                            json s = json::object();
                            if (svc->hostname()) s["hostname"] = svc->hostname()->UTF8Characters();
                            s["port"] = svc->port();
                            s["connectionType"] = svc->connectionType();
                    ...
                    servers["smtp"] = smtpArr;
                    providerJson["servers"] = servers;
                    resp["provider"] = providerJson;
                } else {
                    resp["provider"] = nullptr;
                }
                cout << resp.dump() << "\n";
                cout.flush();
            }

            if (type == "query-capabilities") {
                string requestId = packet.count("requestId") ? packet["requestId"].get<string>() : "";
                json resp = {{"type", "capabilities-result"}, {"requestId", requestId}};
                json caps = json::object();
                // Report capabilities based on the foreground worker's session state
                caps["idle"] = true; // IDLE is always attempted
                caps["condstore"] = true; // CONDSTORE is used when available
                caps["syncInProgress"] = (fgWorker != nullptr);
                resp["capabilities"] = caps;
                cout << resp.dump() << "\n";
                cout.flush();
            }

            if (type == "subscribe-folder-status") {
                string requestId = packet.count("requestId") ? packet["requestId"].get<string>() : "";
                json resp = {{"type", "folder-status"}, {"requestId", requestId}};
                json statuses = json::array();
                if (packet.count("folderIds")) {
                    MailStore statusStore;
                    for (auto & folderId : packet["folderIds"]) {
                        string id = folderId.get<string>();
                        json status = {{"folderId", id}};
                        // Query the folder from the database to get unread counts
                        try {
                            auto query = Query().equal("id", id);
                            auto folders = statusStore.findAll<Folder>(query);
                            if (!folders.empty()) {
                                status["localStatus"] = folders[0]->localStatus();
                            }
                        } catch (...) {
                            // If folder query fails, just return the id
                        }
