...
void TaskProcessor::performRemote(Task * task) {
    ...
    try {
        ...
        if (task->shouldCancel()) {
            ...
            } else if (cname == "SearchMessagesTask") {
                performRemoteSearchMessages(task);

            } else if (cname == "FetchQuotaTask") {
                performRemoteFetchQuota(task);
...
void TaskProcessor::performRemoteSearchMessages(Task * task) {
    json & data = task->data();

    string query = data["query"].get<string>();
    string folderId = data.count("folderId") ? data["folderId"].get<string>() : "";

    IMAPSession imapSession;
    MailUtils::configureSessionForAccount(imapSession, account);

    ErrorCode err = ErrorNone;
    imapSession.connectIfNeeded(&err);
    if (err != ErrorNone) {
        throw SyncException("imap", ErrorCodeToTypeMap[err], false);
    }

    // Determine the folder path to search in
    String * folderPath = nullptr;
    if (!folderId.empty()) {
        auto folder = store->find<Folder>(Query().equal("id", folderId));
        if (folder) {
            folderPath = AS_MCSTR(folder->path());
        }
    }
    if (!folderPath) {
        folderPath = MCSTR("INBOX");
    }

    // Build a content search expression
    IMAPSearchExpression * expression = IMAPSearchExpression::searchContent(AS_MCSTR(query));

    IndexSet * resultUIDs = imapSession.search(folderPath, expression, &err);
    if (err != ErrorNone) {
        throw SyncException("imap", ErrorCodeToTypeMap[err], false);
    }

    // Convert UIDs to a JSON array
    json uids = json::array();
    if (resultUIDs) {
        for (unsigned int i = 0; i < resultUIDs->count(); i++) {
            Range range = resultUIDs->allRanges()[i];
            for (uint64_t uid = range.location; uid < range.location + range.length; uid++) {
                uids.push_back(uid);
            }
        }
    }

    data["resultUIDs"] = uids;
    data["resultCount"] = uids.size();

    logger->info("-- Search completed: {} results for '{}'", uids.size(), query);
}

void TaskProcessor::performRemoteFetchQuota(Task * task) {
    json & data = task->data();
    ...
    // Fetch quota using getQuota (usage and limit in KB)
    uint32_t usage = 0;
    uint32_t limit = 0;
    imapSession.getQuota(&usage, &limit, &err);

    if (err != ErrorNone) {
        // Quota not supported is not a fatal error
        if (err == ErrorCapability) {
            data["supported"] = false;
            logger->info("-- Quota not supported by server");
            return;
        }
        throw SyncException("imap", ErrorCodeToTypeMap[err], false);
    }
    ...
    logger->info("-- Quota fetch completed: {} / {} KB", usage, limit);
}
