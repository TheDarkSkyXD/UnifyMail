...
const json MakeOAuthRefreshRequest(string provider, string clientId, string refreshToken) {
    // Gmail: route through Cloudflare Worker proxy (secret stays server-side)
    string gmailProxyUrl = MailUtils::getEnvUTF8("GMAIL_OAUTH_PROXY_URL");
    if (provider == "gmail" && gmailProxyUrl != "") {
        json requestBody;
        requestBody["refresh_token"] = refreshToken;
        string payloadStr = requestBody.dump();

        CURL * curl_handle = CreateJSONRequest(
            gmailProxyUrl + "/auth/gmail/refresh", "POST", "", payloadStr.c_str()
        );
        return PerformJSONRequest(curl_handle);
    }

    // Microsoft (office365/outlook): direct token refresh
