...
//  mailcore-napi
//
//  N-API binding for quick SMTP connection testing from Node.js.
//
...
class TestSMTPWorker : public Napi::AsyncWorker {
public:
    TestSMTPWorker(Napi::Env env, Napi::Promise::Deferred deferred,
    ...
    void Execute() override {
        ...
        if (err == ErrorNone) {
            // Also try login if credentials were provided
            if (!username_.empty()) {
        ...
        if (err == ErrorNone) {
            ...
            if (errDesc) {
                ...
                errorMessage_ = "SMTP connection failed with error code " + std::to_string(err);
            }
        }
...
// testSMTPConnection(opts): Promise<{success, error?}>
Napi::Value TestSMTPConnection(const Napi::CallbackInfo& info) {
    ...
    auto* worker = new TestSMTPWorker(env, deferred,
...
void InitSMTP(Napi::Env env, Napi::Object exports) {
    exports.Set("testSMTPConnection", Napi::Function::New(env, TestSMTPConnection));
}
