...
import { Model } from 'unifymail-exports';
...
export default class MailsyncBridge {
  ...
  _lastWait: number;
  _responseEmitter = new EventEmitter();
  ...
  toggleVerboseLogging() {
    const { configDirPath } = AppEnv.getLoadSettings();
    let message = localized('Thank you for helping debug UnifyMail. UnifyMail will now restart.');
    let phrase = 'disabled';
    ...
    if (AppEnv.config.get(VERBOSE_UNTIL_KEY)) {
      ...
      message =
        ...
        `all network traffic to your mail providers and will be quite slow. Restart UnifyMail ` +
        `and wait for your problem to occur, and then submit mailsync-***.log files located ` +
        `in the directory: \n\n${configDirPath}.\n\nUnifyMail will now restart.`;
    }
  ...
  sendMessageToAccount(accountId, json) {
    if (!this._clients[accountId]) {
      ...
      return AppEnv.showErrorDialog({
        title: localized(`UnifyMail is unable to sync %@`, emailAddress),
        message: localized(
  ...
  /**
   * Send a request to a specific account's mailsync process and wait for a response.
   * Uses a requestId to correlate the request with the response.
   */
  async sendRequest(accountId: string, message: object, timeoutMs = 10000): Promise<any> {
    const requestId = Utils.generateTempId();
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this._responseEmitter.removeAllListeners(`response:${requestId}`);
        reject(new Error(`Mailsync request timed out after ${timeoutMs}ms`));
      }, timeoutMs);

      this._responseEmitter.once(`response:${requestId}`, (resp) => {
        clearTimeout(timer);
        if (resp.error) {
      ...
      this.sendMessageToAccount(accountId, { ...message, requestId });
    });
  }
  ...
  async resetCacheForAccount(account, { silent }: { silent?: boolean } = {}) {
    ...
    if (!silent) {
      AppEnv.showErrorDialog({
        ...
        message: localized(
          `UnifyMail is clearing its cache %@. Depending on the size of the mailbox, this may take a few seconds or a few minutes. An alert will appear when cleanup is complete.`,
    ...
    try {
      ...
      if (!silent) {
        AppEnv.showErrorDialog({
          ...
          message: localized(
            `UnifyMail reset the local cache for %@ in %@ seconds. Your mailbox will now begin to sync again.`,
            account.emailAddress,
      ...
      AppEnv.showErrorDialog({
        ...
        message: localized(`UnifyMail was unable to reset the local cache. %@`, error),
      });
    } finally {
  ...
  async _launchClient(account: Account, keys, { force }: { force?: boolean } = {}) {
    ...
    client.on('close', ({ code, error, signal }: MailsyncProcessExit) => {
      ...
      const isAuthFailure =
        `${error}`.includes('Response Code: 401') || // UnifyMail services
        `${error}`.includes('Response Code: 403') || // UnifyMail services
  ...
  _onIncomingMessages = msgs => {
    for (const msg of msgs) {
      ...
      // Route request/response protocol messages (detect-provider, capabilities, folder-status, etc.)
      if (type && (type.endsWith('-result') || type === 'folder-status')) {
        const responseKey = json.requestId ? `response:${json.requestId}` : `response:${type}`;
        this._responseEmitter.emit(responseKey, json);
        continue;
      }
