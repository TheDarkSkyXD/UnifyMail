...
const bundleIdentifier = 'com.UnifyMail.UnifyMail';
...
export class DefaultClientHelperWindows implements DCH {
      ...
      (err1, stdout1) => {
        ...
        exec(
          ...
          (err2, stdout2) => {
            ...
            callback(output.includes('UnifyMail'));
          }
        );
      }
    );
  ...
  async resetURLScheme() {
    // On Windows 11 21H2+ (with April 2023 update), we can deep link directly to UnifyMail's
    ...
    shell.openExternal('ms-settings:defaultapps?registeredAppUser=UnifyMail');
  }
      ...
      async (err, didMakeDefault) => {
        ...
        if (!didMakeDefault) {
          const { response } = await require('@electron/remote').dialog.showMessageBox({
            ...
            defaultId: 0,
            message: localized(
              'Visit Windows Settings to finish making UnifyMail your mail client'
            ),
            detail: localized(
              "Click 'Open Settings' to open Windows Settings where you can set UnifyMail as your default email app."
            ),
          });
          if (response === 0) {
            // On Windows 11 21H2+ (with April 2023 update), this deep links directly to
            // UnifyMail's default app settings. On older versions, falls back to Default Apps.
            shell.openExternal('ms-settings:defaultapps?registeredAppUser=UnifyMail');
          }
        }
        callback(null);
...
export class DefaultClientHelperLinux implements DCH {
  ...
  isRegisteredForURLScheme(scheme: string, callback: (registered: boolean | Error) => void) {
    ...
    exec(`xdg-mime query default x-scheme-handler/${scheme}`, (err, stdout) =>
      err ? callback(err) : callback(stdout.trim() === 'UnifyMail.desktop')
    );
  }
  ...
  registerForURLScheme(scheme: string, callback = (error?: Error) => {}) {
    exec(`xdg-mime default UnifyMail.desktop x-scheme-handler/${scheme}`, err =>
      err ? callback(err) : callback(null)
    );
  }
}
