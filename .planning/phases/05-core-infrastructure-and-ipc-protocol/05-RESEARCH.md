...
## ADDENDUM: Deep Dive — Account/Identity JSON Shapes

*Added: 2026-03-02 (second research pass)*

### Line 1: Account JSON

The TypeScript `Account` model serializes via `.toJSON()` which iterates `static attributes` using each attribute's `jsonKey`. `KeyManager.insertAccountSecrets()` injects secrets into `settings` before serialization.

**Complete field list:**

| JSON key | Type | Required by C++ | Notes |
|---|---|---|---|
| `id` | string | Yes (`valid()`) | Account UUID |
| `aid` | string | No | `accountId` attribute serializes as `aid` (jsonKey override) |
| `v` | number | No | `version` attribute serializes as `v` (jsonKey override) |
| `metadata` | array | No | `pluginMetadata` serializes as `metadata`. Usually `[]` |
| `name` | string | No | Display name |
| `provider` | string | Yes (`valid()`) | `"gmail"`, `"imap"`, `"yahoo"`, `"yandex"`, `"office365"`, `"outlook"`, `"eas"` |
| `emailAddress` | string | No | The email address |
| `settings` | object | Yes (`valid()`) | See nested fields below |
| `label` | string | No | Display label (defaults to emailAddress) |
| `autoaddress` | object | No | `{ type: "cc"|"bcc", value: string }` |
| `aliases` | array | No | Array of strings |
| `defaultAlias` | string/null | No | Default send-as alias |
| `syncState` | string | No | `"ok"`, `"invalid"`, or `"sync_error"` |
| `syncError` | object/null | No | `{ code, error, signal }` or null |
| `authedAt` | number | No | Unix timestamp float (seconds). `val.getTime() / 1000.0` |
| `color` | string | No | Hex color or `""` |
| `__cls` | string | No | Always `"Account"` |
...
| `imap_port` | number/string | Yes | C++ handles both via `stoi` coercion |
| `imap_username` | string | Yes | IMAP login username |
| `imap_password` | string | If no `refresh_token` | Injected by KeyManager |
| `imap_allow_insecure_ssl` | boolean | Yes | Must be boolean |
| `imap_security` | string | Yes | `"SSL / TLS"`, `"STARTTLS"`, or `"none"` |
| `smtp_host` | string | Yes | SMTP server hostname |
| `smtp_port` | number/string | Yes | C++ handles both via `stoi` coercion |
| `smtp_username` | string | No | Falls back to `""` |
| `smtp_password` | string | If no `refresh_token` | Injected by KeyManager |
| `smtp_allow_insecure_ssl` | boolean | Yes | Must be boolean |
| `smtp_security` | string | Yes | `"SSL / TLS"`, `"STARTTLS"`, or `"none"` |
| `refresh_token` | string | If no passwords | OAuth2 refresh token. Injected by KeyManager |
| `refresh_client_id` | string | No | OAuth2 client ID for token refresh |
| `container_folder` | string | No | IMAP container folder path. Falls back to `""` |

**C++ `valid()` enforcement:** `id` + `settings` + `provider` must be present. At least one of `refresh_token` OR `imap_password`. At least one of `refresh_token` OR `smtp_password`. `imap_port`, `imap_host`, `imap_username`, `smtp_port`, `smtp_host` required. Both `imap_allow_insecure_ssl` and `smtp_allow_insecure_ssl` must be boolean.

**Rust deserialization notes:**
- `aid` is the JSON key for accountId (not `accountId`)
- `v` is the JSON key for version (not `version`)
- `metadata` is the JSON key for pluginMetadata
- `authedAt` is `f64` (Unix timestamp seconds)
- `settings.imap_port`/`smtp_port` may be number or string — use `serde_json::Value` or custom deserializer
- Use `#[serde(default)]` liberally for optional fields

### Line 2: Identity JSON

Plain TypeScript interface (NOT a Model subclass). `JSON.stringify(this.identity)` directly. Can be literal `"null"`.

| JSON key | Type | Required by C++ | Notes |
|---|---|---|---|
| `id` | string | No | Identity ID |
| `token` | string | Yes (`valid()`) | API token. Re-injected from keychain at runtime |
| `firstName` | string | No | User's first name |
| `lastName` | string | No | User's last name |
| `emailAddress` | string | Yes (`valid()`) | Identity email address |
| `stripePlan` | string | No | e.g. `"Basic"`, `"Pro"` |
| `stripePlanEffective` | string | No | Effective plan |
| `featureUsage` | object | No | Map of feature key → usage object |
| `createdAt` | string | No | ISO 8601 datetime |

**C++ `valid()` returns false if either `emailAddress` or `token` is missing.**

The Rust binary must handle the literal string `"null"` as line 2 meaning "no identity" — deserialize as `Option<Identity>`.

---

## ADDENDUM: Deep Dive — Complete Schema SQL (V1–V9)

*Added: 2026-03-02 (second research pass)*
*Source: `app/mailsync/MailSync/constants.h` — verbatim extraction*

### Connection PRAGMAs (always applied on open)
...
Plus `busy_timeout` = 10 seconds (set via rusqlite API, not PRAGMA).

### V1_SETUP_QUERIES — Initial Schema
...
### V3_SETUP_QUERIES (time-consuming — prints "Running Migration")
...
**Note:** V5 was skipped (no DDL). Migration jumps from `version < 4` to `version < 6`.

### V6_SETUP_QUERIES (destructive — drops and recreates Event)
...
### ACCOUNT_RESET_QUERIES (bound to accountId via `?`)
...
After reset: VACUUM and reset cursor (`_State` key `cursor-<accountId>` → `"0"`).

---

## ADDENDUM: Deep Dive — Bridge Environment, Error Handling, and Restart Logic

*Added: 2026-03-02 (second research pass)*

### Environment Variables Set by mailsync-process.ts

| Variable | Value | Source |
|---|---|---|
| `CONFIG_DIR_PATH` | User data dir (e.g. `~/.config/UnifyMail-dev`) | Constructor arg |
| `GMAIL_CLIENT_ID` | `process.env.MS_GMAIL_CLIENT_ID` or hardcoded fallback | `onboarding-constants.ts` |
| `GMAIL_OAUTH_PROXY_URL` | Hardcoded proxy URL | `onboarding-constants.ts` |
| `IDENTITY_SERVER` | `'unknown'` in main process; `rootURLForServer('identity')` in renderer | Conditional |
| All parent `process.env` | Inherited | Spread operator |

### Binary Path Resolution

1. `<resourcePath>/mailsync.exe` (Windows) or `mailsync.bin` (Unix) — with `app.asar` → `app.asar.unpacked`
2. `<resourcePath>/mailsync/Windows/x64/Release/mailsync.exe` — dev build fallback
3. `<projectRoot>/scripts/mock-mailsync.js` — dev mock fallback
4. If none found: throws error

### Command-Line Arguments by Mode

| Mode | Args | Account on stdin? | Called by |
|---|---|---|---|
| `migrate` | `--mode migrate [--verbose]` | No | `application.ts` startup |
| `sync` | `--mode sync [--verbose] [--info email]` | Yes (full handshake) | `MailsyncBridge._launchClient()` |
| `reset` | `--mode reset [--verbose] [--info email]` | Yes (full handshake) | `MailsyncBridge.resetCacheForAccount()` |
| `test` | `--mode test [--verbose] [--info email]` | Yes (full handshake) | `onboarding-helpers.ts` |

### stdin Buffer: `highWaterMark = 1MB` (handles large draft HTML)

### Exit Code Handling

**`_spawnAndWait` modes (migrate, reset, test):**
- Exit 0: success, response = parsed last JSON line
- Exit non-0 with JSON: structured error (`response.error` mapped to `LocalizedErrorStrings`)
- Exit non-0 non-JSON: raw output becomes error message

**`sync` mode:**
- `signal === 'SIGTERM'`: no restart (intentional kill)
- Normal exit: crash recorded, `ensureClients()` restarts unless too many failures

### Restart Logic (CrashTracker)

- **5+ crashes within 5 minutes** → permanent failure, sets `SYNC_STATE_AUTH_FAILED` or `SYNC_STATE_ERROR`
- **Fewer crashes** → automatic restart via `ensureClients()` (throttled to 100ms)
- Crash key = `JSON.stringify({ id, settings })` — changing settings resets crash history
- User "Try Again" calls `forgetCrashes()` + `_launchClient(force: true)`

### stderr Handling

- `sync` mode: accumulated into `errBuffer`, becomes error message on non-zero exit
- `_spawnAndWait` modes: mixed into same buffer as stdout; `migrate` mode checks for `"running migration"` / `"running vacuum"` strings to show status window

### No Watchdog Timer

The bridge has NO heartbeat/watchdog. It only reacts to process `'close'`/`'exit'`/`'error'` events. Individual request/response pairs have a 10-second timeout but this does not kill/restart the process.
