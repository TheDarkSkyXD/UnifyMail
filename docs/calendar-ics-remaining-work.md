...
- ✅ Updated `UnifyMail-calendar.tsx` drag persistence to use ICS helpers
...
- ✅ Exported `ICSEventHelpers` from `UnifyMail-exports`
...
**Issue**: `_onDeleteSelectedEvents` in `UnifyMail-calendar.tsx:230-254` doesn't check for recurring events or show the dialog.

**Current behavior**: Deletes the event directly without asking about series vs occurrence.

**Files to modify**:
- `app/internal_packages/main-calendar/lib/core/UnifyMail-calendar.tsx`
...
**Implementation**:
```typescript
import { ICSEventHelpers } from 'UnifyMail-exports';
...
**Current behavior** (`UnifyMail-calendar.tsx:682-690`):
```typescript
...
**Issue**: `UnifyMail-calendar.tsx:130` uses deprecated React lifecycle method.
