...
module.exports = grunt => {
  ...
  function runCopyPlatformSpecificResources(buildPath, electronVersion, platform, arch, callback) {
    ...
    const winResourcesSource = path.resolve(grunt.config('buildDir'), 'resources', 'win');
  ...
  grunt.config.merge({
    packager: {
      ...
      icon: {
        ...
        win32: path.resolve(grunt.config('buildDir'), 'resources', 'win', 'UnifyMail.ico'),
        linux: undefined,
      }[platform],
      name: {
        darwin: 'UnifyMail',
      ...
      extendInfo: path.resolve(grunt.config('buildDir'), 'resources', 'mac', 'extra.plist'),
